...
import { Brain, Code, Users, Lightbulb, TrendingUp, Clock } from 'lucide-react';
...
export default function SkillsEngine() {
    ...
    useEffect(() => {
        ...
        const topSkills = getTopSkills(professionData.skill_automation_breakdown, 8);
        ...
        const formattedSkills = topSkills.map(([skillName, skillData]) => {
            ...
            return {
                name: skillName.length > 100 ? skillName.substring(0, 100) + " ..." : skillName,
    ...
    if (error || skills.length === 0) {
        return (
            <div className="relative bg-white/5 backdrop-blur-xl border border-white/10 rounded-xl p-6">
                <div className="flex items-center justify-center h-64">
                    <div className="text-zinc-400">{error || "No skills data available"}</div>
                </div>
            </div>
        );
    }
                                    ...
                                    <div className="space-y-2">
                                        <div>
                                            <div className="flex justify-between text-xs text-zinc-400 mb-1">
                                                ...
                                                <span>{skill.automation}%</span>
                                            </div>
                                            <div className="h-1.5 bg-white/10 rounded-full overflow-hidden">
                                                <div
                                                    className={`h-full ${skill.automation > 70 ? 'bg-red-500' :
                                                        skill.automation > 40 ? 'bg-yellow-500' :
                                                            'bg-green-500'
                                                        }`}
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="bg-white/5 rounded-lg p-4">
                            ...
                            <div className="text-2xl font-bold text-white">
                                {Object.keys(professionData.skill_automation_breakdown || {}).length}
                            </div>
                        </div>
                        <div className="bg-white/5 rounded-lg p-4">
                            <div className="flex items-center gap-2 mb-2">
